...
class XiachufangtypeSpiderSpider(scrapy.Spider):
	...
	def parse(self, response):
		...
		for node in node_list_1:
			cates_list_info = node.xpath("./div[@class='cates-list-info ml15 float-left]/h3/text()").extract()[0]
			node_list_2 = node.xpath(".//div[@class='cates-list-all clearfix hidden']/ul")
			for n in node:
				cates_list_first = n.xpath("./h4/text()").extract()
			...
			print(node.xpath(
				".//div[@class='cates-list-all clearfix hidden']/ul/li/a/text()").extract())
			print("------------test2------------------")
